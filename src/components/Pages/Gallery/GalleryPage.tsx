...
/* ── Data ─────────────────────────────────────────────────── */
const galleryImages = [
    { src: "/472855498_1113804890464912_7048434061283229523_n.jpg", cat: "Community" },
    { src: "/570085856_1976814893717423_4351859646516135459_n.jpg", cat: "Events" },
    { src: "/597922916_4227217690925495_4207115085969201556_n.jpg", cat: "Campus" },
    { src: "/600825102_1569959977483625_8463712704209570580_n.jpg", cat: "Events" },
    { src: "/605124268_4466273233594105_7958836570682652007_n.jpg", cat: "Community" },
    { src: "/bamhs.png", cat: "Campus" },
    { src: "/bamhs2.png", cat: "Campus" },
    { src: "/photo-1622737133809-d95047b9e673.jpeg", cat: "Portrait" },
    { src: "/WhatsApp Image 2025-11-15 at 17.54.13_b4dda1d4.jpg", cat: "Community" },
    { src: "/12345.png", cat: "Events" },
    { src: "/Anas.png", cat: "Portrait" },
    { src: "/Anas2.png", cat: "Portrait" },
    { src: "/me-anas.png", cat: "Portrait" },
];

const FILTERS = ["All", "Campus", "Events", "Community", "Portrait"] as const;
    ...
    return (
        <div className="three-xl-section-setup pb-20 space-y-16">
            ...
            >
                ...
                />
                {/* glow blobs */}
                <div className="absolute -top-24 -left-24 h-72 w-72 rounded-full blur-3xl opacity-25"
                    style={{ background: "rgba(46,139,87,1)" }} />
                <div className="absolute -bottom-16 -right-16 h-56 w-56 rounded-full blur-3xl opacity-20"
                ...
                <div className="relative z-10 px-7 py-12 sm:px-12 sm:py-16">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
                        ...
                        <p className="mt-5 max-w-2xl text-sm sm:text-lg text-primary2-100/75 leading-relaxed">
                            Curated alumni moments through an editorial masonry grid, featured collections,
                            and visual narratives crafted for future archive expansion.
                        </p>
                    </motion.div>
                    ...
                    >
                        {galleryStats.map(({ value, label }) => (
                            <div key={label} className="rounded-2xl border px-4 py-4 text-center"
            ...
            <section>
                <FadeUp>
                    <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h2 className="text-2xl sm:text-3xl font-bold text-primary2-900">Photo Archive</h2>
                ...
                >
                    {filtered.map((img, idx) => (
                        <motion.div
                            ...
                            className="group relative overflow-hidden rounded-2xl border border-surface-300/60 bg-surface"
                        >
                            <Image
                                src={img.src}
                                alt={`${img.cat} photo ${idx + 1}`}
                                width={800}
                                height={1000}
                            ...
                            <div className="absolute inset-0 flex flex-col justify-end p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                                style={{ background: "linear-gradient(to top, rgba(4,26,18,0.85) 0%, transparent 60%)" }}>
            ...
            {/* ═══ 3. FEATURED COLLECTIONS (shadcn Carousel) ═══════ */}
            <FadeUp>
                <div className="mb-5">
                    <h2 className="text-2xl sm:text-3xl font-bold text-primary2-900">Featured Collections</h2>
                    <p className="mt-1 text-sm text-muted-foreground">Curated albums by moment, era, and community contribution.</p>
                </div>
                <HorizontalSnapCarousel>
                    {featuredCollections.map((item) => (
                        <Card key={item.title} className="h-full border-primary2-200/60 hover:-translate-y-1 transition-transform duration-200">
                            <CardContent className="p-6">
                                ...
                                <h3 className="mt-5 text-lg font-semibold text-primary2-900 leading-snug">{item.title}</h3>
                                <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{item.note}</p>
                                <p className="mt-4 text-xs font-medium text-primary2-600">{item.count}</p>
                            </CardContent>
                        </Card>
                    ))}
                </HorizontalSnapCarousel>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_0.6fr] gap-5">
                    <Card className="border-primary2-200/60">
                        <CardContent className="p-6 sm:p-8">
                            <h3 className="text-xl font-bold text-primary2-900">Visual Story Strips</h3>
                            <p className="mt-2 text-sm text-muted-foreground">
                                Sequence memories as mini-documentaries — assembly, classroom, reunion, service.
                            </p>
                            ...
                            <div className="grid sm:grid-cols-3 gap-3">
                                {[
                                    ...
                                    <div key={phase} className="rounded-xl border border-surface-300/60 bg-primary2-50/50 p-4">
                                        <p className="text-xs uppercase tracking-[0.14em] text-primary2-600 font-medium">{phase}</p>
                                        <p className="mt-2 text-sm text-primary2-900 font-medium">{desc}</p>
                                    </div>
                                ))}
                            </div>
                        </CardContent>
                    ...
                    <Card className="border-primary2-200/60">
                        <CardContent className="p-6 sm:p-8 flex flex-col">
                            ...
                            <p className="mt-2 text-sm text-muted-foreground flex-1">
                                Submit your best BAMHS moments with context to enrich the alumni archive permanently.
                            </p>
            ...
            <FadeUp>
                <div className="rounded-3xl border border-primary2-200/60 px-6 py-5 flex flex-wrap items-center gap-3"
                    ...
                    <p className="text-sm text-primary2-900 flex-1 min-w-0">
                        <strong>Next upgrade-ready:</strong> AI-assisted photo tagging, year-wise smart filters, lightbox viewer, and event-based dynamic albums.
                    </p>
                </div>
            </FadeUp>
        </div>
